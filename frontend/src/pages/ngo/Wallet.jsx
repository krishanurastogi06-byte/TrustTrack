...
import { Wallet as WalletIcon, Copy, ArrowDownLeft, ArrowUpRight, CheckCircle2, AlertCircle } from "lucide-react";
...
function Wallet() {
    const { account, connect, disconnect, provider, isConnected, chainId, isMetaMaskAvailable, error, isInitialized } = useWallet({ role: "ngo" });
    ...
    const [walletConflictWithDonor, setWalletConflictWithDonor] = useState(false);

    const short = account ? `${account.slice(0, 6)}...${account.slice(-4)}` : "-";
    const shortSaved = savedWalletAddress
        ? `${savedWalletAddress.slice(0, 6)}...${savedWalletAddress.slice(-4)}`
        : "Not saved";

    const isSameAsSaved = useMemo(() => {
        if (!account || !savedWalletAddress) return false;
        return account.toLowerCase() === savedWalletAddress.toLowerCase();
    }, [account, savedWalletAddress]);
    ...
    useEffect(() => {
        async function loadProfile() {
            try {
                ...
                setWalletConflictWithDonor(Boolean(profile?.walletConflictWithDonor));
            } catch (err) {
    ...
    async function handleConnect() {
        ...
        try {
            ...
            const connectedAddress = connected?.account;
            ...
            // Auto-save the connected address to backend
            setIsSaving(true);
            try {
                const updated = await ngoService.updateWalletAddress(connectedAddress);
                setSavedWalletAddress(updated?.walletAddress || connectedAddress.toLowerCase());
                setWalletConflictWithDonor(false);
                setSaveSuccess("Wallet connected and saved successfully.");
            } catch (backendErr) {
                // Wallet still connected even if backend save fails
                setSavedWalletAddress(connectedAddress.toLowerCase());
                setSaveError("Wallet connected but failed to save to profile: " + (backendErr?.message || "Unknown error"));
            }
        } catch (err) {
    ...
    async function handleDisconnect() {
        ...
        setSaveSuccess("");
        try {
            // Clear wallet from backend (optional - user's preference)
            // You can either clear it or leave it saved for re-connection
            // For now, we'll just disconnect locally
            disconnect();
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,430px)_minmax(0,1fr)] gap-6 items-start">
                <Card className="w-full relative overflow-hidden group border-none shadow-[0_8px_30px_-4px_rgba(79,70,229,0.15)] bg-gradient-to-br from-indigo-900 via-indigo-800 to-indigo-950 text-white">
                    ...
                    <div className="relative z-10">
                        ...
                        {isConnected && (
                            <>
                                <div className="bg-white/10 backdrop-blur-md rounded-2xl p-4 mb-8 border border-white/10 flex items-center justify-between">
                                    <div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <Button
                                ...
                                disabled={isConnecting || isSaving || !isInitialized}
                        ...
                        {isConnected && !isSameAsSaved && !saveSuccess && !saveError && (
                            <p className="mt-2 text-xs text-amber-200 flex items-center gap-1">
                                <AlertCircle size={14} /> Connected wallet is different from saved profile address.
                            </p>
                        )}
                        {walletConflictWithDonor && (
                            <p className="mt-2 text-xs text-red-200 flex items-center gap-1">
                                <AlertCircle size={14} /> Warning: donor and NGO are using the same wallet address.
                            </p>
                        )}
                    </div>
                </Card>
